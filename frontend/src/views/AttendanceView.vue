<template>
  <div class="space-y-8 animate-fade-in pb-12 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6">
    ...
    <div class="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6 border-b border-gray-200/80 pb-6">
      <div class="max-w-2xl">
        <div class="flex items-center gap-3 mb-2">
          <div :class="showArchived ? 'bg-amber-100 text-amber-700' : 'bg-primary-100 text-primary-700'" class="p-2 rounded-xl">
            <svg v-if="showArchived" class="w-6 h-6" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" d="M20.25 14.15v4.25c0 1.094-.787 2.036-1.872 2.118l-12.14.936A2.25 2.25 0 014.25 19.2V5.208a2.25 2.25 0 011.987-2.236l.93-.07a7.5 7.5 0 1114.357 5.629l-1.274 5.619z" />
            </svg>
        ...
        <p class="text-sm text-gray-500 leading-relaxed">
          {{ showArchived ? 'Browse historical attendance records and verification logs.' : 'Monitor operational facilities and process live teacher attendance checks.' }}
        </p>
      </div>
      ...
      <div class="flex flex-wrap items-center gap-3 w-full lg:w-auto">
        <div class="relative flex-1 sm:flex-initial group">
          <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
            <svg class="h-4 w-4 text-gray-400 group-focus-within:text-primary-500 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <button 
          ...
          class="flex-1 sm:flex-initial flex items-center justify-center gap-2 text-xs uppercase tracking-wider font-bold px-5 py-2.5 rounded-xl shadow-sm transition-all duration-200 active:scale-[0.98]"
          :class="showArchived ? 'bg-amber-600 text-white hover:bg-amber-700 ring-1 ring-amber-600' : 'bg-gray-900 text-white hover:bg-gray-800 ring-1 ring-gray-900'"
        >
          <svg v-if="showArchived" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />
          </svg>
          <svg v-else class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 8h14M5 8a2 2 0 110-4h14a2 2 0 110 4M5 8v10a2 2 0 002 2h10a2 2 0 002-2V8m-9 4h4" />
          </svg>
          {{ showArchived ? '📊 Active View' : '📦 Archive View' }}
        </button>

        <button 
          v-if="!showArchived"
          @click="generateTodayAttendance" 
          class="flex-1 sm:flex-initial text-xs uppercase tracking-wider font-bold bg-emerald-600 text-white hover:bg-emerald-700 px-5 py-2.5 rounded-xl shadow-sm transition-all duration-200 active:scale-[0.98] ring-1 ring-emerald-600"
        >
          Generate Matrix
        </button>
        ...
        <button 
          ...
          class="flex-1 sm:flex-initial flex items-center justify-center gap-2 text-xs uppercase tracking-wider font-bold bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed px-5 py-2.5 rounded-xl shadow-sm transition-all duration-200 active:scale-[0.98] ring-1 ring-blue-600"
        >
    ...
    <!-- Archive Info Banner -->
    <div v-if="showArchived" class="bg-amber-50 border border-amber-200 rounded-2xl p-4 flex items-center justify-between">
      <div class="flex items-center gap-3">
        ...
        <div>
          <p class="text-sm font-bold text-amber-800">Viewing Archived Records</p>
          <p class="text-xs text-amber-700">These records are permanently archived and cannot be restored. Switch to "Active View" to manage current attendance.</p>
        </div>
      </div>
      <button 
        @click="showArchived = false" 
        class="text-xs font-bold text-amber-700 hover:text-amber-900 bg-white/50 hover:bg-white px-4 py-2 rounded-lg transition-colors"
      >
        Switch to Active →
      </button>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm ring-1 ring-gray-200 overflow-hidden">
      <div class="px-6 py-4 border-b border-gray-200 bg-gray-50/50 flex items-center justify-between">
        <div class="flex items-center gap-3">
          <div class="flex h-8 w-8 items-center justify-center rounded-lg bg-white ring-1 ring-gray-200 shadow-sm">
            <svg v-if="showArchived" class="h-4 w-4 text-amber-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 8h14M5 8a2 2 0 110-4h14a2 2 0 110 4M5 8v10a2 2 0 002 2h10a2 2 0 002-2V8m-9 4h4" />
            </svg>
            <svg v-else class="h-4 w-4 text-primary-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />
            </svg>
          </div>
          <div>
            <h3 class="text-sm font-bold text-gray-900 flex items-center gap-2">
              {{ showArchived ? 'Archived Records' : 'Active Records' }}
              ...
              <span class="inline-flex items-center rounded-md bg-primary-50 px-2 py-0.5 text-xs font-medium text-primary-700 ring-1 ring-inset ring-primary-500/10">
                {{ filteredAttendanceList.length }} entries
              </span>
            </h3>
          </div>
        </div>
        
        <!-- Archive All Button (only in active view) -->
        <div class="flex items-center gap-2">
          <button 
            v-if="!showArchived && filteredAttendanceList.length > 0"
            @click="archiveAllRecords" 
            class="inline-flex items-center gap-1.5 text-xs font-bold text-amber-700 hover:text-amber-900 bg-amber-50 hover:bg-amber-100 px-3 py-1.5 rounded-lg transition-colors"
          >
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 8h14M5 8a2 2 0 110-4h14a2 2 0 110 4M5 8v10a2 2 0 002 2h10a2 2 0 002-2V8m-9 4h4" />
            </svg>
          ...
          <span v-if="showArchived" class="inline-flex items-center gap-1.5 text-xs font-bold text-gray-400 bg-gray-100 px-3 py-1.5 rounded-lg">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div class="relative">
        <div v-if="loading" class="absolute inset-0 z-10 bg-white/80 backdrop-blur-sm flex flex-col justify-center items-center min-h-[300px]">
          ...
          <p class="mt-4 text-xs font-bold text-gray-500 uppercase tracking-wider">Loading...</p>
        </div>
        ...
        <div v-if="filteredAttendanceList.length === 0 && !loading" class="text-center py-24 animate-fade-in px-6">
          ...
          <h3 class="text-sm font-semibold text-gray-900">{{ showArchived ? 'No Archived Records' : 'No Records Found' }}</h3>
          <p class="mt-1 text-sm text-gray-500 max-w-sm mx-auto">
            {{ showArchived ? 'No attendance records have been archived for this date.' : 'Generate attendance records to start tracking.' }}
          </p>
          ...
          <table class="min-w-full divide-y divide-gray-200" id="attendance-table-element">
            <thead class="bg-gray-50">
              <tr>
                ...
                <th scope="col" class="px-3 py-3.5 text-left text-[10px] font-bold text-gray-500 uppercase tracking-wider">Room</th>
                <th scope="col" class="px-3 py-3.5 text-left text-[10px] font-bold text-gray-500 uppercase tracking-wider">Teacher</th>
                ...
                <td class="whitespace-nowrap py-4 pl-3 pr-6 class-exclude-pdf text-center">
                  ...
                  <div v-if="!showArchived" class="inline-flex items-center gap-2">
                    ...
                    <button 
                      ...
                      class="rounded-md bg-white px-2 py-1.5 text-gray-400 ring-1 ring-inset ring-gray-300 hover:bg-amber-50 hover:text-amber-600 hover:ring-amber-300 shadow-sm transition-all"
                  ...
                  <div v-else class="inline-flex items-center gap-2">
                    <span class="inline-flex items-center gap-1.5 text-[10px] font-bold text-gray-400 bg-gray-50 px-3 py-1.5 rounded-lg ring-1 ring-inset ring-gray-200">
...
// Individual Archive - Only archiving, no unarchive
const toggleArchiveRecord = async (att: DailyAttendance) => {
  if (att.is_archived) {
    alert('This record is already archived.')
    return
  }
  
  if (!confirm(`Archive this record for ${att.teacher_name}? This action cannot be undone.`)) return
  
  const previousState = att.is_archived
  att.is_archived = true
  
  try {
    await post(`/attendance/${att.id}/archive`, { is_archived: 1 })
    await fetchAttendance()
  } catch (err) {
    att.is_archived = previousState
    console.warn('Archive failed:', err)
    alert('Failed to archive record. Please try again.')
  }
}

// Bulk Archive All - Only archiving, no unarchive
const archiveAllRecords = async () => {
  const count = filteredAttendanceList.value.length
  if (count === 0) return
  
  if (!confirm(`Archive all ${count} records for ${selectedDate.value}? This action cannot be undone.`)) return
...
const downloadPDFReport = () => {
  ...
  doc.text(`Date: ${selectedDate.value}`, 14, 28)
  doc.text(`View Mode: ${showArchived.value ? 'Archived Records (Read Only)' : 'Active Records'}`, 14, 34)
  doc.text(`Summary: Total: ${totalClasses.value} | Present: ${presentCount.value} | Absent: ${absentCount.value} | Pending: ${pendingCount.value}`, 14, 42)
  ...
  autoTable(doc, {
    startY: 48,
    head: [['Schedule', 'Room', 'Teacher', 'Subject', 'Status', 'Remarks']],
    body: tableRows,
    theme: 'striped',
  ...
  doc.save(`Attendance_${selectedDate.value}_${showArchived.value ? 'archived' : 'active'}.pdf`)
}
